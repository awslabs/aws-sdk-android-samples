...
3. Get CognitoIdentity from credentialsProvider and use it to attach pricipal policy to the IotAndroidClient. Create IoT policy as specified in step 4 of [Using the sample](https://github.com/awslabs/aws-sdk-android-samples/tree/desokroshan-patch-2/AndroidPubSub#using-the-sample) section above. 

```
// Setup the logins map
String token = IdentityManager.getDefaultIdentityManager().getCurrentIdentityProvider().getToken();
Map<String, String> logins = new HashMap();
// fill it with Cognito User token
// Key must be of the form "cognito-idp.<REGION>.amazonaws.com/<COGNITO_USER_POOL_ID>"
// COGNITO_USER_POOL_ID is generated by amplify cli in step 1 above
logins.put("cognito-idp.us-east-1.amazonaws.com/us-east-1_UmiXXXXXX", token);
